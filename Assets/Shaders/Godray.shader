...
Shader "Custom/Blinking GodRays" {  
    Properties {  
		...
		_MaxBlinkFactor("Blink factor bias", float) = 1.0
		_MinBlinkFactor("Blink factor bias", float) = 0.0
    ...
    SubShader{  
       ...
       #include "UnityCG.cginc"  
        ...
        float   _Multiplier;  
        float   _Bias;  
		float	_MaxBlinkFactor;
		float	_MinBlinkFactor;
        ...
        {   
			...
			float blinkFactor = nfadeout * _Multiplier * wave;
			if (blinkFactor < _MinBlinkFactor)
			{
				blinkFactor = _MinBlinkFactor;
			}
			if (blinkFactor > _MaxBlinkFactor)
			{
				blinkFactor = _MaxBlinkFactor;
			}
			o.color = blinkFactor * _Color;
